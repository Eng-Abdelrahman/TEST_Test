...
namespace _3aqarak.MVC.Controllers
{
    ...
    public class RentContractController : Controller
    {
        ...
        public RentContractController(ITransService transService, ILandsDemandsService landDemand, IShopDemandService shopDemand, IVillasDemandService villasDemand, IShopAvailableService shopAvailable, IAvailableLandsSevice availableLand, IVillasAvailablesService villasAvailable, IAccountingService accountingService, IexpectedContractService expectedService, IPreviewService previewService, IDemandService demandService, IStaticService staticService, IClientService clientService, IConfirmation conf, IAvailableService availableService, IRentContractService rentService)
        {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var rentcontractVM = new RentContractViewModel()
            {
                Cats = await _rentService.GetCats(),
            };
            return View(rentcontractVM);
        }
        ...
        private async Task<DataTableViewModel> GetTableData(DataTableViewModel tableData, DateTime? from, DateTime? to,int catId)
        {
            ...
            List<RentContractViewModel> entityList = Mapper.Map<List<RentHeaderDto>, List<RentContractViewModel>>(await _rentService.GetContracts(from, to,catId));
            foreach (var item in entityList)
            {
                item.BuyerName = (await _clientService.FindClientByID(item.FK_RentAgreements_Clients_Buyer)).Name;
                item.SellerName = (await _clientService.FindClientByID(item.FK_RentAgreements_Clients_Seller)).Name;
            ...
            {
                entityList = entityList.Where(e => e.BuyerName.Contains(tableData.SearchValue) || e.SellerName.Contains(tableData.SearchValue) || e.BuyerIdNumber.Contains(tableData.SearchValue) || e.SellerIdNumber.Contains(tableData.SearchValue)).ToList();
            }
            ...
            if (!(string.IsNullOrEmpty(tableData.SortColumn) && string.IsNullOrEmpty(tableData.SortColumnDir)))
            {
                ...
                {
                    entityList = entityList.OrderBy(e => e.PK_RentAgreements_Id).ToList();
                }
                else
                {
                    entityList = entityList.OrderByDescending(e => e.PK_RentAgreements_Id).ToList();
                }
            ...
            tableData.RentContracts = entityList;
        ...
        public async Task<ActionResult> AddContract(string id, string type, string isareement)
        {
            if (HttpContext.Items["msg"] != null)
            {
                Session["message"] = HttpContext.Items["msg"].ToString();
            ...
            if (transaction.TransCode != TranscatTypes.Rental)
            {
            ...
            var newContract = new RentContractViewModel();
            newContract.TypeOfContract= GetContractType(bool.Parse(isareement));
            newContract.EmpName = userName;
            newContract.FK_RentAgreements_Users_EmpId = ((UserDto)Session["User"]).PK_Users_Id;
            newContract.FK_RentAgreements_Clients_Buyer = expected.FK_ExpectContract_Clients_Buyer;
            newContract.FK_RentAgreements_Clients_Seller = expected.FK_ExpectContract_Clients_Seller;
            newContract.BuyerName = buyer.Name;
            newContract.SellerName = seller.Name;
            newContract.SellerAddress = seller.Address;
            newContract.SellerIdNumber = seller.IdNumber;
            newContract.BuyerIdNumber = buyer.IdNumber;
            newContract.BuyerAddress = buyer.Address;
            newContract.Basis = await _rentService.GetPaymentBasis();
            ...
            newContract.DemandCat = (await _previewService.FindById(expected.FK_ExpectedContract_Preview_Id)).Category_Id;
        ...
        public async Task<ActionResult> SaveContract(RentContractViewModel contractVM)
        {
            var userId = ((UserDto)Session["User"]).PK_Users_Id;
            if (contractVM.RentalStartDate >= contractVM.RentalEndDate)
            {
                return Json(new { valid = false, message = "تاريخ نهاية الايجار لابد ان يكون اكبر من بدايته!" }, JsonRequestBehavior.AllowGet);
            }
            if (contractVM.RentalStartDate >= contractVM.DateNxtRent)
            {
                return Json(new { valid = false, message = "تاريخ الايجار القادم لابد ان يكون اكبر من بدايته!" }, JsonRequestBehavior.AllowGet);
            }
            Regex regex = new Regex("^([0-9]{14})$");
            if (!regex.IsMatch(contractVM.SellerIdNumber) || !regex.IsMatch(contractVM.BuyerIdNumber))
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    contractVM.ContractUrl = ((List<string>)Session["RentContractImagePathList"])[0];
                    Session["RentContractImagePathList"] = null;
                }
                //var use = GetUsetId(contractVM.DemandUnits_Id, contractVM.DemandCat);
                _conf = await _rentService.SaveContract(Mapper.Map<RentContractViewModel, RentHeaderDto>(contractVM), Mapper.Map<RentContractViewModel, RentDetailsDto>(contractVM), userId);
                if (_conf.Valid)
                {
        ...
        public async Task<ActionResult> EditContract(string id)
        {
            ...
            if (HttpContext.Items["Disable"] != null)
            {
                ViewBag.Disable = true;
            }
            var contractId = int.Parse(id);
            var contractVM = Mapper.Map<RentHeaderDto, RentContractViewModel>(await _rentService.FindHeaderByID(contractId));
            contractVM.BuyerName = (await _clientService.FindClientByID(contractVM.FK_RentAgreements_Clients_Buyer)).Name;
            contractVM.SellerName = (await _clientService.FindClientByID(contractVM.FK_RentAgreements_Clients_Seller)).Name;
            var userName = ((UserDto)Session["User"]).FirstName + " " + ((UserDto)Session["User"]).LastName;
            contractVM.EmpName = userName;
            contractVM.Basis = await _rentService.GetPaymentBasisId(contractVM.FK_RentAgreementHeader_PaymentBasis_Id);
            contractVM.DetailContent = (await _rentService.FindDetailByID(contractVM.PK_RentAgreements_Id)).DetailContent;
            return View(contractVM);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> UpdateContract(RentContractViewModel contractVM)
        {
            var userId = ((UserDto)Session["User"]).PK_Users_Id;
            if (contractVM.RentalStartDate >= contractVM.RentalEndDate)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                //var use = GetUsetId(contractVM.DemandUnits_Id, contractVM.DemandCat);
                _conf.Valid = await _rentService.UpdateContract(Mapper.Map<RentContractViewModel, RentHeaderDto>(contractVM), Mapper.Map<RentContractViewModel, RentDetailsDto>(contractVM), userId);
        ...
        {
            var userId = ((UserDto)Session["User"]).PK_Users_Id;
            var contract = await _rentService.FindHeaderByID(int.Parse(id));
            var valid = await _rentService.DeleteContract(int.Parse(id), userId);
            if (valid)
            {
                ...
                var deletedCompCommission = await _rentService.DeleteRelatedCompCommission(contract.PK_RentAgreements_Id);
                var deletedEmpCommission = await _rentService.DeleteRelatedEmpCommission(contract.PK_RentAgreements_Id);
            }
        ...
        public ActionResult EndedRentalsIndex(string rentals)
        {
            var rentalListDto = Newtonsoft.Json.JsonConvert.DeserializeObject<List<RentHeaderDto>>(rentals);
            var rentalListViewModel = Mapper.Map<List<RentHeaderDto>, List<RentContractViewModel>>(rentalListDto);

            return View(rentalListViewModel);
        }
        public async Task<ActionResult> EndRentContract(int contractId)
        {

            var userId = ((UserDto)Session["User"]).PK_Users_Id;
            var header = await _rentService.FindHeaderByID(contractId);
            var ended = await _rentService.EndRentContract(header);
            if (ended)
            {
                await ReleaseDemand(header.DemandUnits_Id, header.DemandCat, userId);
                await  ReleaseAvailable(header.AvailableUnits_Id, header.AvailableCat, userId);

            }
            return Json(ended, JsonRequestBehavior.AllowGet); ;
        ...
        public ActionResult UploadImages()
        {
            try
            {
                if (Session["RentContractImagePathList"] != null && ((List<string>)Session["RentContractImagePathList"]).Count > 0)
                {
                    var path = ((List<string>)Session["RentContractImagePathList"])[0];
                    var photo = Directory
                ...
                if (files.Count < 1)
                {
                    return Json(new { valid = false, message = "لابد من تحميل صورة لا اكثر ولا اقل" }, JsonRequestBehavior.AllowGet);
                }
                //HttpPostedFileBase file = files[i];
                _conf = _rentService.SavePhotos(files);
                Session["RentContractImagePathList"] = _conf.UrlList;
        ...
        public async Task<ActionResult> GetPDF(int id)
        {
            ...
            {
                var fileStream = new FileStream(HostingEnvironment.MapPath("~/" + (await _rentService.FindHeaderByID(id)).ContractUrl),
                                                 FileMode.Open,
                                                 FileAccess.Read
                                               );
            ...
            {
                var pdfPath = ((List<string>)Session["RentContractImagePathList"])[0];
        ...
        public ActionResult RentalsToCollect(string rentcollects)
        {
            var rentalsTocollectDto = Newtonsoft.Json.JsonConvert.DeserializeObject<List<RentHeaderDto>>(rentcollects);
            var rentalsToCollectVM = Mapper.Map<List<RentHeaderDto>, List<RentContractViewModel>>(rentalsTocollectDto);
            return View(rentalsToCollectVM);
        }
        public async Task<ActionResult> CollectRentalFees(DateTime newDate, int id)
        {
            var userId = ((UserDto)Session["User"]).PK_Users_Id;
            var nextRental = await _rentService.FindHeaderByID(id);
            if (newDate == null || newDate < nextRental.DateNxtRent || newDate > nextRental.RentalEndDate)
            {
                _conf.Message = "الميعاد غير سليم او اقل من تاريخ الدفعه السابقه";
                return Json(new { Valid = false, message = _conf.Message }, JsonRequestBehavior.AllowGet);
            }
            await _rentService.UpdateNextRent(nextRental, newDate, userId);
        ...
        public async Task<bool> UpdateAvailableTransaction(int catId, int AvailableId)
        {
            ...
            if (catId == Categories.Apartements)
            {
                updatedAvailable = await _availableService.UpdateAvailableTransaction(AvailableId, TranscatTypes.Rental);
            }
            //this's for Villas
            else if (catId == Categories.Villas)
            {
                updatedAvailable = await _villasAvailableService.UpdateAvailableTransaction(AvailableId, TranscatTypes.Rental);
            }
            else if (catId == Categories.Lands)
            {
                updatedAvailable = await _availableLandService.UpdateLandAvailableTransaction(AvailableId, TranscatTypes.Rental);
            }
            else if (catId == Categories.Shops)
            {
                updatedAvailable = await _shopAvailableService.UpdateShopAvailableTransaction(AvailableId, TranscatTypes.Rental);
            }
        ...
        public async Task<bool> UpdateDemandTransaction(int catId, int previewId)
        {
            ...
            if (catId == Categories.Apartements)
            {
                updatedDemand = await _demandService.UpdateDemandTransaction((await _previewService.FindById(previewId)).DemandUnit_Id, TranscatTypes.Rental);
            }
            //this's for Villas
            else if (catId == Categories.Villas)
            {
                updatedDemand = await _villasDemandService.UpdateDemandTransaction((await _previewService.FindById(previewId)).DemandUnit_Id, TranscatTypes.Rental);
            }
            //this's for Land
            else if (catId == Categories.Lands)
            {
                updatedDemand = await _landDemandService.UpdateLandDemandTransaction((await _previewService.FindById(previewId)).DemandUnit_Id, TranscatTypes.Rental);
            }
            //this's for Shop
            else if (catId == Categories.Shops)
            {
                updatedDemand = await _shopDemandService.UpdateDemandTransaction((await _previewService.FindById(previewId)).DemandUnit_Id, TranscatTypes.Rental);
            }
